...
namespace TuitionWaiverDistribution.Algorithms {
    public class KnapsackBranch<T> {
        ...
        public KnapsackBranch(List<SackItem<T>> items, int weightLimit) {
            state = new(this);
        ...
        public KnapsackBranch<T> SetDebug(bool state) {
            debug = state;
            return this;
        }

        private class State {
            public KnapsackBranch<T> Parent { get; }
            ...
            public State(KnapsackBranch<T> parent) {
                Parent = parent;
            }
